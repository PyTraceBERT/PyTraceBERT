...
from transformers import (TrainingArguments, Trainer, BertTokenizer,
                          DataCollatorForLanguageModeling, BertForPreTraining)
...
def load_data(args, valid_ratio=0.1):
...
def parse_args():
    ...
    # args.add_argument("-with_cuda", "--with_cuda", type=bool, default=True, help="wether train with cuda")
    args.add_argument("-pretrain_data", "--pretrain_data", type=str,
                      default="./dataset/pretrain/templates_train_FEP.csv",
                      help="pre-train data directory")

    args.add_argument("-outfolder", "--outfolder", type=str,
                      default="./output/pytracebert/pretrain_fep/", help="Folder name to save the models.")
...
def pretraining(args):
    ...
    def tokenize_function(tracebacks):
        ...
        # tokenized_examples['input_ids'] = [input_id for input_id in tokenized_examples['input_ids'] if len(input_id) >= min_length]
